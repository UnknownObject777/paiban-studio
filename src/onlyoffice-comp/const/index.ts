import { DocumentType } from "../internal/editor/types.js";
...
// paiban-studio 注：渲染层走 127.0.0.1 回环静态服务器（站点根 = 项目根，见 src/main.ts），
// SDK 资产位于项目根 public/packages/ 下，因此站点相对根前缀为 /public/packages。
const DEFAULT_ONLYOFFICE_ROOT = `/public/packages/onlyoffice/${DEFAULT_ONLYOFFICE_VERSION}`;
...
export function resolveSiteUrl(origin: string, path: string): string {
  // paiban-studio 注：上游只豁免 https?://；本项目渲染层走 paiban:// 自定义协议，
  // 主线程已解析为 paiban:// 绝对地址后再进 Worker 会被二次拼接，因此豁免任意 scheme。
