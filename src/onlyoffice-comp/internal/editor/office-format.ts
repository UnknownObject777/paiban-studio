import { parseCsvBuffer } from "./utils.js";
...
async function loadExcelJS() {
  // paiban-studio 注：vendored 后项目未安装 exceljs（仅 CSV→XLSX 路径使用，docx 预览走不到）。
  // 用变量间接持有 specifier，避免 tsc NodeNext 在编译期解析该裸包名。
  const specifier = "exceljs/dist/exceljs.min";
