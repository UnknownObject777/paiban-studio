...
function decompressWithVendoredBrotli(buffer: ArrayBuffer) {
  return import("../vendor/brotli-dec/index.js").then(({ brotliDecompress }) =>
...
async function decompressBrotli(buffer: ArrayBuffer) {
  if (canDecompressBrotliInBrowser()) {
    try {
      ...
      // paiban-studio 调试注：临时暴露 stream 解压失败原因（上游为静默回退）
      console.error("[x2t-assets] DecompressionStream 解压失败，回退 vendored brotli:", e);
    }
  } else {
    console.error("[x2t-assets] DecompressionStream 不可用（isSecureContext?）");
  }
