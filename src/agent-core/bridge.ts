...
//
// 职责：
//   - createAgentSession() 内嵌，tools 白名单只放自研八工具（裁剪内置文件工具，防绕过编辑内核）
...
工作方式（必须遵守）：
1. 修改已有文档只能调用 doc_edit；从零生成新文档调用 doc_generate；用模板生成新文档（用户说"用某模板做一份/套模板生成"）调用 template_instantiate。不要尝试直接读写文件。
...
2. 生成路径三分支：从零 → doc_generate；用模板（用户说"用某模板做一份"）→ 先 template_read 拿占位符清单，把用户给的信息填进 values 再调 template_instantiate（没给的占位符保留不填，会原样留在文档里提示人工补；「金额（大写）」栏先 amount_words）；改已有 → doc_outline + doc_edit。
3. markdown 约定：# 文档题目（只一个）、## 章节标题、GFM 表格做报价表/字段表、**粗体** 强调。
4. 领域结构（起草时遵循；缺失信息用「×××」占位，不要编造具体公司名/金额）：
...
export const TOOL_WHITELIST = ['doc_outline', 'doc_edit', 'doc_generate', 'template_instantiate', 'template_read', 'ruleset_read', 'version_store', 'amount_words'];
...
export function summarizeArgs(args: any): string {
  ...
  if (args.templateId && args.values) {
    const n = typeof args.values === 'object' && args.values ? Object.keys(args.values).length : 0;
    return `实例化模板 ${args.templateId}（${n} 个占位符）`;
  }
...
export function summarizeToolResult(e: any): string {
  ...
  if (Array.isArray(d.replaced)) {
    // template_instantiate：文档名 + 版本 + 替换/未匹配数
    const errs = Array.isArray(d.errors) ? d.errors.length : 0;
    return `实例化《${d.name ?? '文档'}》${d.version?.id ? ' · ' + d.version.id : ''}，替换 ${d.replaced.length} 处${errs ? `，${errs} 处未匹配` : ''}`;
  }
