// docx-core/numbering.js — numbering.xml 多级编号封装（spec 标记的首要风险点之一，单独成模块重点测试）。
//
// 职责：
...
import {
  el, textEl, isElement, getAttr, setAttr,
...
// 公文多级标题预设：一、 / （一） / 1. / （1）
export const GONGWEN_HEADING_LEVELS = [
...
function ensureNumberingPart(docx) {
  ...
  const { parseXml } = numberingModule;
...
import * as numberingModule from './xml.js';

function nextIds(root) {
...
function buildLvl(def) {
  ...
  if (runProps) {
    ...
    if (runProps.eastAsia || runProps.ascii) {
      ...
      rPr['w:rPr'].push(el('w:rFonts', fonts));
    }
    if (runProps.sizePt) {
      rPr['w:rPr'].push(el('w:sz', { 'w:val': Math.round(runProps.sizePt * 2) }));
    }
 ...
 */
export function defineNumbering(docx, levels) {
...
/** 给段落挂载编号引用。ilvl 默认 0。 */
export function setParagraphNumbering(pNode, numId, ilvl = 0) {
