...
//   agent 六工具直接 execute（不依赖 LLM）：doc_outline / doc_edit / doc_generate / template_read / ruleset_read / version_store
...
test('agent 工具：doc_outline → doc_edit → version_store 全链路（无 LLM）', async () => {
  ...
  assert.deepEqual(tools.map((t) => t.name), ['doc_outline', 'doc_edit', 'doc_generate', 'template_read', 'ruleset_read', 'version_store']);
