...
用单摆测量本地重力加速度，**验证**单摆周期公式 T=2π√(L/g) 的适用性，掌握累积放大法测周期的技巧。

## 二、实验原理

摆角小于 5° 时单摆做简谐运动，周期 T 与摆长 L 满足 g=4π²L/T²。为减小计时误差，测量 n=50 个全振动的总时间再求平均周期。
...
// 涉外收入申报单（收入方向，与境外汇款申请书的付款方向相对）：
// 申报号码由银行编制；收入款区分结汇/原币入账；交易编码按收入性质选择。
const FX_IN_PLACEHOLDER_MD = `# 涉外收入申报单

申报号码：{{申报号码}}（银行编制）　　　　日期：{{日期}}
...
const jobs = [
  ...
  // 外汇单另一形态：涉外收入申报单（收入方向），占位符同样在字段表格单元格内。
  { markdown: FX_IN_PLACEHOLDER_MD, rulesetId: 'fx-form-default', name: '涉外收入申报单占位符模板（演示）.docx', templateName: '涉外收入申报单占位符模板（演示）' },
];
